...
contract FlashlabsNft is ERC1155, AccessControlEnumerable, FlashlabsUpgradeable, FlashlabsNftMemoryLayout {
    event TokenAdded(uint256 indexed id, string name, address minter);
    event TokenLocked(uint256 indexed id, address owner, address lockedFor, uint256 amount);
    ...
    function _initializer(bytes calldata data) internal override {
        ...
        _setupRole(TOKEN_PROPOSER_ROLE, msg.sender);
    ...
    function lockFor(address operator, uint256 id, uint256 amount) public {
        ...
        emit TokenLocked(id, _msgSender(), operator, amount);
    }
